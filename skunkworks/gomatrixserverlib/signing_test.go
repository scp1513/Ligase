...
type MyMessage struct {
	Unsigned   *jsonRaw.RawMessage `json:"unsigned"`
	Content    *jsonRaw.RawMessage `json:"content"`
	Signatures *jsonRaw.RawMessage `json:"signatures,omitempty"`
}
...
func TestSignJSONWithUnsigned(t *testing.T) {
	...
	entityName := "example.com"
	keyID := KeyID("ed25519:my_key_id")
	content := jsonRaw.RawMessage(`{"signed":"data"}`)
	unsigned := jsonRaw.RawMessage(`{"unsigned":"data"}`)
	...
	newUnsigned := jsonRaw.RawMessage(`{"different":"data"}`)
